...
| | `mail.smtp_port` | `25` | 사내 릴레이는 대개 25. 465 면 `tls` 로 봄 |
| | `mail.security` | `auto` | `auto` · `none` · `starttls` · `tls`. `auto` 는 릴레이가 STARTTLS 를 알리면 쓰고 아니면 평문 |
| | `mail.skip_tls_verify` | `false` | 사내 인증서가 사설일 때만 |
| | `mail.username` · `mail.password` | (없음) | 인증 없는 릴레이가 흔하므로 **선택**. 비밀번호는 암호화 저장되고 되읽히지 않음(화면에는 "설정됨") |
| | `mail.from_address` · `mail.from_name` | (없음) · `Ptium` | 보내는 사람. 주소를 비우면 `ptium@<릴레이 호스트>` |
| | `mail.base_url` | (없음) | 메일 속 링크가 가리킬 이 서비스의 주소. 비우면 `PUBLIC_BASE_URL` |
| | `mail.timeout_seconds` | `10` | SMTP 한 단계의 제한 시간(1~120) |
| | `mail.notify_generation_completed` | `true` | 덱이 완성되면 요청한 사람에게 |
| | `mail.notify_generation_failed` | `true` | 생성·재작성이 실패로 멈추면 요청한 사람에게 |
| | `mail.notify_comment` | `true` | 공유 링크로 리뷰어가 의견을 남기면 덱 작성자에게 |
...
덱 하나를 쓰는 데 자체 호스팅 모델은 1~5분이 걸리고, 리뷰를 부탁한 링크에 의견이 달리는 것은 며칠
뒤일 수도 있습니다. 이 절이 비어 있는 동안 그 사이를 메우는 것은 사람이 화면을 새로고침하는 일입니다.
사내 서비스가 모두 같은 표준(MAIL-STANDARD)을 따르므로 **서비스 설정 → 메일 알림**에 릴레이를
적으면 그때부터 기다리던 일이 메일로 옵니다. 설정 키 이름은 다른 서비스와 같습니다.

**보내는 이벤트.** "무언가 바뀜"은 보내지 않습니다. 이 메일이 오지 않으면 화면을 계속 새로고침하게
되는 세 가지만 보냅니다 — 종류별로 끌 수 있습니다.

| 이벤트 | 받는 사람 | 왜 |
| --- | --- | --- |
| 덱이 완성됨 (`generation.completed`) | 요청한 사람 | 몇 분짜리 대기. 자리를 떠나도 됨 |
| 생성이 실패로 멈춤 (`generation.failed`) | 요청한 사람 | 화면에 보이는 사유와 같은 말로. 안 오면 돌아와서야 앎 |
| 리뷰어가 의견을 남김 (`comment.created`) | 덱 작성자 | 링크를 보낸 사람이 기다리는 바로 그것. 의견 본문은 담지 않음 |

자기가 한 일은 자기에게 가지 않습니다(작성자가 워크스페이스에서 다는 답글은 아무에게도 가지
않습니다). 메일은 **배경에서** 보내므로 릴레이가 죽어 있어도 덱 생성이나 의견 남기기는 평소처럼 끝나고,
실패는 발송 기록에만 남습니다. 한 번 실패하면 2초 뒤 한 번 더 시도합니다.

**릴레이 설정.** 관리자 콘솔 **서비스 설정 → 메일 알림**에서 릴레이 호스트를 적고 **메일 알림 사용**을
켜서 저장합니다. 사내 릴레이는 포트 25 · 인증 없음 · TLS 없음이 흔하고 그것이 기본값이므로 대개
호스트만 적으면 됩니다. 인증이 필요한 릴레이면 사용자 이름과 비밀번호를 넣습니다 — 비밀번호는
암호화되어 저장되고 화면에는 "설정됨"만 보이며, 바꿀 때만 새 값을 넣습니다. 폐쇄망에서는 릴레이로
사내 메일 서비스 `postra` 를 가리키면 알림이 밖으로 나가지 않습니다. 재시작이 필요 없습니다.

**시험 발송.** 릴레이 설정은 한 번에 맞는 일이 드뭅니다. 저장한 뒤 같은 화면의 **시험 메일 보내기**를
누르면 저장된 설정으로 실제 한 통을 보내고(받는 사람을 비우면 내 계정 주소로) 결과를 그 자리에서 보여
줍니다. 릴레이가 거절하면 릴레이가 한 말이 그대로 나옵니다 — `SMTP connection failed` 는 호스트·포트,
`STARTTLS` 는 보안 방식, `RCPT TO refused` 는 릴레이가 그 수신자를 받지 않는 것입니다. API 로는
`POST /api/v1/admin/mail/test` 에 `{"recipient":"me@corp.example"}` 입니다.

**발송 기록.** 같은 화면 아래 **발송 기록**에 시도마다 남습니다 — 언제, 어떤 이벤트로, 누구에게, 제목이
무엇이었고, 되었는지. 성공과 실패가 모두 남으므로 "안 왔다"는 문의에 답할 수 있습니다. 본문은 담지
않습니다. API 로는 `GET /api/v1/admin/mail/deliveries?status=failed&limit=50` 입니다. 표는
`mail_deliveries` 이며 백업에 같이 들어갑니다.

**확인.** 설정을 저장하고 시험 발송이 도착하면, 덱 하나를 생성해 완성 메일이 오고 링크가 편집기로
...
| 시험 메일이 `mail_send_failed` — `SMTP connection failed` | 서비스 설정 → 메일 알림 | 호스트·포트가 맞는지, 이 컨테이너에서 릴레이의 25번 포트가 열리는지(`nc -vz relay 25`) |
| 시험 메일이 `STARTTLS failed` 또는 인증서 오류 | 서비스 설정 → 메일 알림 | 사설 인증서면 **인증서 검증 건너뛰기**, 릴레이가 암호화를 안 하면 보안을 `none` 으로 |
| 알림이 오지 않는데 발송 기록에도 없음 | 서비스 설정 → 메일 알림 | `mail.enabled` 가 꺼져 있거나 그 이벤트 스위치가 꺼진 것, 또는 받는 사람 계정에 메일 주소가 없는 것(관리 → 사용자) |
