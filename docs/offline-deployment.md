...
```powershell
.\load-ptium-1.69.15.ps1 -Archive .\ptium-1.69.15.tar.gz
```

```bash
./load-ptium-1.69.15.sh ptium-1.69.15.tar.gz
```
...
```bash
sha256sum -c ptium-1.69.15.tar.gz.sha256
gzip -dc ptium-1.69.15.tar.gz | docker load
docker image inspect ptium-1.69.15:latest ptium:1.69.15 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-1.69.15.yml up -d
docker compose --env-file .env -f docker-compose.ptium-1.69.15.yml ps
...
kubectl apply -f ptium-1.69.15.kubernetes.yaml
```
...
The manifest asks for 192Mi and limits the pod to 1Gi. What that headroom is
for, measured on this build in a container held to that limit:

| Work | Time | Peak memory |
| --- | --- | --- |
| Four 40-slide decks, a 1376×768 photograph on every slide, printed to PDF at once | 7.1s | 639 MiB |
| Twelve exports at once (pptx, PDF and handout, six-slide decks) | 0.2s | 49 MiB |
| 44,547 mixed requests over a minute (477 generations, 477 deletes, 477 prints) | — | no failures |

The picture is what decides the first row. A photograph of that size is 4 MB
once it is decoded for drawing, and four documents printing forty slides each at
the same time is what fills the pod. Text-only decks are the second row: the
same twelve exports finish in a fifth of a second and barely move the memory.

Printing is the heaviest thing the server does per request: it draws every slide
and embeds a font. It is bounded — the font is embedded once per document and a
...
Reading an uploaded file is the other peak: the package is held in memory while it is read, so leave
headroom above `generation.max_template_mb` (32 MB by default; 64 MB is the most
the setting accepts).

One template at the maximum size peaks at 441 MiB and an imported deck of the
same size at 255 MiB, so both share the budget above: a template read costs all
three units, an import costs two, a PDF one, a `.pptx` three.

Unbounded, four templates or six imports at once killed a pod held to 1Gi and
took every request in flight with them. Bounded, eight 60 MB templates and
twelve 60 MB imports sent together all succeed — the imports in six seconds,
never passing a third of the limit. An upload still waiting after 30 seconds is
answered `503 templates_busy` or `503 import_busy` rather than left hanging.
...
to one this site has left. Records written before 1.52.1 carry no build, which
