...
```powershell
.\load-ptium-1.56.1.ps1 -Archive .\ptium-1.56.1.tar.gz
```

```bash
./load-ptium-1.56.1.sh ptium-1.56.1.tar.gz
```
...
```bash
sha256sum -c ptium-1.56.1.tar.gz.sha256
gzip -dc ptium-1.56.1.tar.gz | docker load
docker image inspect ptium-1.56.1:latest ptium:1.56.1 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-1.56.1.yml up -d
docker compose --env-file .env -f docker-compose.ptium-1.56.1.yml ps
...
kubectl apply -f ptium-1.56.1.kubernetes.yaml
```
