...
```powershell
.\load-ptium-0.80.0.ps1 -Archive .\ptium-0.80.0.tar.gz
```

```bash
./load-ptium-0.80.0.sh ptium-0.80.0.tar.gz
```
...
```bash
sha256sum -c ptium-0.80.0.tar.gz.sha256
gzip -dc ptium-0.80.0.tar.gz | docker load
docker image inspect ptium-0.80.0:latest ptium:0.80.0 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-0.80.0.yml up -d
docker compose --env-file .env -f docker-compose.ptium-0.80.0.yml ps
...
kubectl apply -f ptium-0.80.0.kubernetes.yaml
```
