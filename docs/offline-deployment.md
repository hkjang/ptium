...
```powershell
.\load-ptium-1.49.0.ps1 -Archive .\ptium-1.49.0.tar.gz
```

```bash
./load-ptium-1.49.0.sh ptium-1.49.0.tar.gz
```
...
```bash
sha256sum -c ptium-1.49.0.tar.gz.sha256
gzip -dc ptium-1.49.0.tar.gz | docker load
docker image inspect ptium-1.49.0:latest ptium:1.49.0 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-1.49.0.yml up -d
docker compose --env-file .env -f docker-compose.ptium-1.49.0.yml ps
...
kubectl apply -f ptium-1.49.0.kubernetes.yaml
```
...
One template at the maximum size peaks at 441 MiB. Ptium reads **one at a time**
for that reason — four at once killed a pod held to 1Gi, and every request in
flight died with it. Uploads queue instead: eight 60 MB templates sent together
all succeed, in sixteen seconds, without the pod passing half its limit. An
upload that waits more than 30 seconds is answered `503 templates_busy` rather
than left hanging.
...
to one this site has left. Records written before 1.49.0 carry no build, which
