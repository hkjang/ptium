...
```powershell
.\load-ptium-1.12.5.ps1 -Archive .\ptium-1.12.5.tar.gz
```

```bash
./load-ptium-1.12.5.sh ptium-1.12.5.tar.gz
```
...
```bash
sha256sum -c ptium-1.12.5.tar.gz.sha256
gzip -dc ptium-1.12.5.tar.gz | docker load
docker image inspect ptium-1.12.5:latest ptium:1.12.5 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-1.12.5.yml up -d
docker compose --env-file .env -f docker-compose.ptium-1.12.5.yml ps
...
kubectl apply -f ptium-1.12.5.kubernetes.yaml
```
