...
```powershell
.\load-ptium-0.9.1.ps1 -Archive .\ptium-0.9.1.tar.gz
```

```bash
./load-ptium-0.9.1.sh ptium-0.9.1.tar.gz
```
...
docker image inspect ptium-0.9.1:latest ptium:0.9.1 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-0.9.1.yml up -d
docker compose --env-file .env -f docker-compose.ptium-0.9.1.yml ps
...
kubectl apply -f ptium-0.9.1.kubernetes.yaml
```
