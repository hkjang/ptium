...
```powershell
.\load-ptium-1.9.0.ps1 -Archive .\ptium-1.9.0.tar.gz
```

```bash
./load-ptium-1.9.0.sh ptium-1.9.0.tar.gz
```
...
```bash
sha256sum -c ptium-1.9.0.tar.gz.sha256
gzip -dc ptium-1.9.0.tar.gz | docker load
docker image inspect ptium-1.9.0:latest ptium:1.9.0 >/dev/null
```
...
```bash
docker compose --env-file .env -f docker-compose.ptium-1.9.0.yml up -d
docker compose --env-file .env -f docker-compose.ptium-1.9.0.yml ps
...
kubectl apply -f ptium-1.9.0.kubernetes.yaml
```
