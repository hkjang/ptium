...
생성이 끝나면 편집기가 열립니다. 화면에 보이는 슬라이드는 모두 **서버가 실제 템플릿으로
그린 것**입니다 — 왼쪽 목록, 편집 화면 옆의 큰 그림, 발표 모드까지 같은 렌더러를 씁니다.
...
텍스트만 아는 클라이언트가 슬라이드를 저장해도 컴포넌트와 이미지는 사라지지 않습니다.
`blocks`·`images`를 언급하지 않은 수정은 저장된 그림을 그대로 유지하며, 지우려면 명시적으로
`"blocks": {}`를 보내야 합니다.
