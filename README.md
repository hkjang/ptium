...
- **이미 있는 자료에서 만들기** — `.pptx` 뿐 아니라 **엑셀·CSV·워드·PDF·마크다운**을 올리면
  슬라이드가 됩니다. 시트는 한 장씩(라벨 한 열 + 숫자 한 열이면 차트, 넓으면 표), 보고서는
  제목이 슬라이드, **PDF는 한 쪽이 한 장**(반복되는 머리글·꼬리글과 쪽 번호는 빼고, 한 장에
  안 들어가는 줄은 발표자 노트로). **각 장에 어느 파일 어디에서 왔는지 출처가 붙고**, 글자가
  없는 스캔본은 빈 덱을 만드는 대신 무엇이 문제인지 알려 줍니다
...
이미 있는 `.pptx`는 **기존 자료 가져오기**로 덱이 됩니다. 발표자료가 아니어도 됩니다 —
`.xlsx` · `.csv` · `.tsv` · `.docx` · `.pdf` · `.md` · `.txt` 가 같은 자리에서 덱이 되고, 각 장에
출처(`!source 매출.xlsx | 분기 실적!A1:B5`)가 붙습니다. `.pptx` 에서는 제목·요점(들여쓰기)·발표자
노트·표·그림과 슬라이드의 성격이 넘어옵니다. 표는 새 덱의 디자인으로 다시 그려지고, 그림은 이미지
