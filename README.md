...
`::image 이름` 이미지, `!notes` 발표 노트. 이것이 전부입니다. 전체 문법과 컴포넌트 목록은
...
`::image 이름`이 코드에 들어갑니다. 레이아웃의 그림 영역, 없으면 가장 넓은 본문 영역에
비율을 유지한 채 가운데를 잘라 들어갑니다.
...
사진·글자 위), 대비 부족(합성 영역 4.5:1 미만), 그리고 제목의 마지막 줄에 한 음절만
남는 줄바꿈입니다. 담기지 않아 잘려나간 문장도 경고로
알립니다 — 작성자가 쓴 것보다 조용히 적게 말하는 슬라이드가 더 나쁩니다.

같은 측정을 기본 제공 30종 전부에 대해 표지·본문·컴포넌트·표·차트를 쓰는 덱으로 테스트에서
