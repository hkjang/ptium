...
- 레이아웃 전환, 발표자 노트 및 PowerPoint 내보내기
...
이미지는 편집기 오른쪽 **이미지** 탭에서 끌어다 놓아 올립니다. **슬라이드에 넣기**를 누르면
현재 캔버스에 자유 배치되고, **코드에 넣기**를 누르면 `::image 이름`이 코드에 들어갑니다.
코드 이미지는 레이아웃의 그림 영역, 없으면 가장 넓은 본문 영역에 비율을 유지한 채 가운데를
잘라 들어갑니다.
...
- 각 슬라이드가 무엇을 담고 있는지 이름으로 보여줍니다(핵심 지표·타임라인·비교·표·이미지 등).
  컴포넌트는 텍스트로 고칠 수 없으므로 **코드** 탭으로 안내하고, 제목·리드·본문을 고쳐도
  지워지지 않습니다.
- 헤더의 상태 표시는 저장된 덱을 실제로 측정한 결과입니다: 결함 0이면 "결함 없음", 아니면
  개수를 눌러 어느 슬라이드의 무엇이 문제인지 보고 바로 그 슬라이드로 이동합니다.
- 발표 모드는 ← → 로 넘기고 ESC 로 닫습니다. 발표 노트가 슬라이드 아래에 함께 나옵니다.
- 계정 메뉴 아래에 실행 중인 빌드 버전이 표시됩니다. 오류를 알릴 때 이 값을 함께 알려주세요.

텍스트만 아는 클라이언트가 슬라이드를 저장해도 컴포넌트·이미지·자유 배치 개체는 사라지지
않습니다. `blocks`·`images`·`elements`를 언급하지 않은 수정은 저장된 그림을 그대로 유지하며,
지우려면 각각 빈 객체나 배열을 명시적으로 보내야 합니다.
