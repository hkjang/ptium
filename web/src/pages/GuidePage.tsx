      ...
      <article className="guide-body">
        ...
        <section id="import" className="guide-section">
          ...
          <p>지난 분기 보고서, 잘 먹힌 제안서, 늘 복사해 쓰는 회사 소개.
            ...
            <code>.pptx</code>를 올리면 Ptium 덱이 됩니다. <b>발표자료가 아니어도 됩니다</b> —
            엑셀(<code>.xlsx</code>)·CSV·워드(<code>.docx</code>)·PDF·마크다운도 슬라이드가 되고,
            각 장에 <b>어느 파일 어디에서 왔는지</b> 출처가 붙습니다.</p>
          <ul className="guide-checklist">
            <li>제목 · 요점(들여쓰기 그대로) · 발표자 노트 · <b>표</b>가 넘어오고, 표는 새 덱의
              디자인으로 다시 그려집니다.</li>
            <li>표지·간지 같은 <b>슬라이드의 성격</b>도 함께 넘어옵니다.</li>
            <li><b>그림도 넘어옵니다</b> — 이미지 라이브러리에 저장되고 새 디자인의 그림 자리에
              들어갑니다. 놓여 있던 좌표는 가져오지 않고, 모든 장에 반복되는 로고와 작은 장식은
              슬라이드의 내용이 아니므로 넣지 않습니다.</li>
            <li><b>차트는 숫자째 넘어옵니다</b> — 세로막대·가로막대는 그대로, 추이는 선 그래프로,
              원형은 점유 막대로. 그리지 못하는 형태는 몇 개였는지 알려 줍니다.</li>
            <li><b>엑셀·CSV</b>는 시트마다 한 장이 됩니다. 라벨 한 열과 숫자 한 열이면 차트로,
              넓으면 표로. <b>워드·마크다운</b>은 제목이 슬라이드, 그 아래 문장이 요점입니다.</li>
            <li><b>PDF</b>는 한 쪽이 한 장이 됩니다. 쪽마다 첫 줄이 제목처럼 보이면 제목이 되고,
              모든 쪽에 반복되는 머리글·꼬리글과 쪽 번호는 빼고, 각 장에 <b>몇 쪽에서 왔는지</b>가
              붙습니다. 스캔한 PDF처럼 글자가 없는 파일은 빈 덱을 만들지 않고 그 이유를 알려 줍니다.</li>
            <li>가져온 뒤에는 보통 덱입니다: 템플릿을 바꾸고, 고치고, 편집기의 <b>AI로 다듬기</b>로
              덱 전체를 다시 쓰게 하세요 — 숫자와 사실은 그대로 두고 제목·문장·구성만 바뀌며,
              이전 상태는 버전 이력에 남습니다.</li>
          </ul>
        </section>
        ...
        <section id="images" className="guide-section">
          ...
          <ul className="guide-checklist">
            ...
            <li><b>말로 시키기</b> — 편집기 위 <b>명령</b>에 <code>3번과 4번 합쳐줘</code>,
              <code>5번 삭제</code>, <code>10분 발표로 맞춰줘</code>처럼 적으면 무엇을 할지 먼저
              보여 주고 적용합니다. 모델을 쓰지 않아 폐쇄망에서도 같습니다.</li>
            <li><b>코드에서</b> — 올린 이미지는 이름이 있고, 소스에 <code>::image 로고</code>로 부릅니다. 설명은 <code>::image 로고 | 2026 브랜드 마크</code>처럼 덧붙이며, 대체 텍스트가 되고 그림 옆에 자리가 남으면 캡션으로도 보입니다.</li>
          </ul>
        ...
        <section id="source" className="guide-section">
          ...
          <dl className="guide-syntax">
            ...
            <div><dt><code>[말](https://…)</code></dt><dd>링크. <code>mailto:</code> 와 다른 슬라이드로 가는 <code>#3</code> 도 됩니다</dd></div>
            <div><dt><code>**굵게**</code> <code>*기울임*</code></dt><dd>줄 안에서 한 단어만 강조</dd></div>
          </dl>
        ...
        <section id="present" className="guide-section">
          ...
          <ul className="guide-checklist">
            ...
            <li><b>내보내기</b> — PPTX로 받습니다. 도형·표·차트가 진짜 개체이고 자리 표시자가 그대로
              남아 PowerPoint에서 계속 편집됩니다. <strong>PDF</strong>는 어디서나 열리는 배포용입니다 — 화면에
              보이는 것과 같은 그림에, 링크와 슬라이드 이동까지 살아 있습니다. 글꼴은 워크스페이스가
              싣고 있는 나눔바른고딕입니다(템플릿은 글꼴 이름만 담고 파일은 담지 않습니다).
              <b>PDF · 발표자 노트</b>는 한 장에 슬라이드와 그 아래 노트를 넣은 유인물입니다.</li>
          </ul>
        </section>
