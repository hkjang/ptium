...
export function EditorPage({ id }: { id: string }) {
   ...
   */
  const rewriteDeck = async (instruction: string) => {
    setRewriteOpen(false)
    setRewriting(true)
    try {
      if (dirty) await save()
      setPresentation(await api.rewritePresentation(id, instruction))
      showToast(instruction
        ? '요청한 내용으로 다시 쓰고 있습니다. 끝나면 이 화면이 바뀝니다.'
        : '덱을 다시 쓰고 있습니다. 끝나면 이 화면이 바뀝니다.')
  ...
  return (
    <main className="editor-page">
      ...
      <RewriteDialog
        open={rewriteOpen}
        busy={rewriting}
        onRewrite={(instruction) => void rewriteDeck(instruction)}
