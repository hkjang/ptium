...
export function AdminUsersPage() {
  ...
  return <AppShell title="사용자 관리" eyebrow="ACCESS CONTROL">
    ...
    <section className="admin-panel users-panel"><div className="users-toolbar"><div className="search-box"><Search size={17} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름 또는 이메일 검색" /></div><div><Select aria-label="역할로 거르기" value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)}><option value="all">모든 역할</option><option value="admin">관리자</option><option value="user">일반 사용자</option></Select><Select aria-label="계정 상태로 거르기" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">모든 상태</option><option value="active">활성</option><option value="suspended">정지됨</option></Select></div></div>
