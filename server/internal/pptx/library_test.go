...
func TestDesignLibraryIsBalanced(t *testing.T) {
	...
	if len(designs) != 40 {
		t.Fatalf("the library ships %d designs, want 40", len(designs))
	}
	families, palettes, keys := map[string]int{}, map[string]int{}, map[string]bool{}
	covers, bodies := map[string]bool{}, map[string]bool{}
	...
	for palette, count := range palettes {
		if count != 4 {
			t.Fatalf("palette %s appears %d times, want 4", palette, count)
		}
	}
	...
	if len(covers) != 6 {
		t.Fatalf("the library ships %d cover compositions, want 6: %v", len(covers), covers)
	}
	if len(bodies) != 3 {
