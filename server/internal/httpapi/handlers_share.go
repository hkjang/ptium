...
// The messages here answer in English like every other handler: they are also
// read from a request log and by whoever is writing against the API. The
// workspace turns them into the reader's words in web/src/api/errors.ts, and
// the shared page is the one page that was not asking it to.
