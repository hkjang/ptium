...
func AuthorMessage(cause error, language string) string {
	...
	case strings.Contains(text, "rewriting a deck needs an ai provider"):
		// Not a misconfiguration. A deployment with no model host writes decks
		// with the offline writer and cannot rewrite one — telling its users to
		// go and ask an administrator to fix a service that is exactly as it
		// shipped sends them after nothing.
		return words.rewriteNeedsModel
...
type failureWords struct {
	...
	// rewriteNeedsModel is the one thing the offline writer will not stand in
	// for: rewriting somebody's deck would replace it with a new one about the
	// brief. The deck is untouched, which is the first thing to say.
	rewriteNeedsModel string
...
func failureWordsKorean() failureWords {
	return failureWords{
		...
		rewriteNeedsModel: "덱을 다시 쓰려면 연결된 AI 모델이 필요합니다. 이 배포는 브리프로 새 덱을 쓰는 내장 생성기만 갖고 있어서, 이미 있는 덱은 다시 쓰지 못합니다. 덱은 그대로입니다.",
...
func failureWordsEnglish() failureWords {
	return failureWords{
		...
		rewriteNeedsModel: "Rewriting a deck needs a connected AI model. This deployment has the built-in writer, which writes a new deck from a brief and does not rewrite an existing one. Your deck is unchanged.",
...
func failureWordsJapanese() failureWords {
	return failureWords{
		...
		rewriteNeedsModel: "デッキを書き直すには接続されたAIモデルが必要です。この配置にはブリーフから新しいデッキを書く内蔵ジェネレーターのみがあり、既存のデッキは書き直せません。デッキはそのままです。",
...
func failureWordsChinese() failureWords {
	return failureWords{
		...
		rewriteNeedsModel: "重写演示文稿需要已连接的 AI 模型。本部署仅有内置生成器，它根据简报撰写新文稿，无法重写现有文稿。您的文稿未被改动。",
